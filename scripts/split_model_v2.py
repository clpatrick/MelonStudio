...
def export_partition(
    ...
    class GPUPartitionModel(torch.nn.Module):
        def __init__(self, full_model, num_layers):
            ...
            # Capture rotary embedding if available (Phi-3, Llama, etc.)
            self.rotary_emb = getattr(full_model.model, 'rotary_emb', None)

        def forward(self, input_ids, attention_mask, position_ids):
            hidden_states = self.embed_tokens(input_ids)
            
            # Compute position embeddings if rotary_emb exists
    ...
    try:
        ...
        # Attention mask should be float type to match model expectations
        dummy_attention_mask = torch.ones(batch_size, seq_length, dtype=torch_dtype)
